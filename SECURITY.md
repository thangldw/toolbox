...
- signed/notarized release artifacts and published SHA-256 checksums
...
Ranh giới chính: validate canonical path ngay trước mutation; project rule fail-closed; executable/argument allowlist chính xác; review foreground; không auto cleanup/overwrite restore/privileged helper/Gatekeeper bypass; evidence local; update endpoint công khai duy nhất theo thao tác người dùng; artifact ký/notarize và checksum SHA-256.
...
境界は mutation 直前の canonical path validation、fail-closed project rule、exact executable/argument allowlist、foreground review、restore overwrite/automatic cleanup/privileged helper/Gatekeeper bypass の禁止、local evidence、user-initiated の単一 public update endpoint、signed/notarized artifact と SHA-256 です。Asset を黙って差し替えず、incident 時は affected release を無効化して advisory と patch release を公開します。
