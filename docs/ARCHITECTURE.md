...
| `ToolboxStorage` | `history.json` | Up to 500 cleanup entries, including Trash move manifests and restore state; written atomically. The current loader returns an empty history when the file is absent or undecodable, so this legacy-format store does not provide Core-style quarantine. |
...
| `ToolboxChanges` | `sessions.json`, `active-snapshot.json`, `active-trace-metadata.json`, `trusted-baseline.json` | Sessions, interrupted-work state, installer metadata, and the user-created baseline; written atomically. Session history is capped at 100. Loaders currently return empty or `nil` for unreadable legacy-format data rather than quarantining it. |
...
| `ToolboxStorage` | `history.json` | Tối đa 500 cleanup entry, gồm Trash move manifest và restore state; được ghi atomic. Loader hiện tại trả history rỗng nếu file không tồn tại hoặc không decode được, nên legacy-format store này không có quarantine như Core. |
...
| `ToolboxStorage` | `history.json` | Trash move manifest と restore state を含む最大 500 cleanup entry。Atomic write です。現在の loader は file 不在または decode 不可で空 history を返すため、この legacy-format store には Core と同じ quarantine はありません。 |
...
| `ToolboxChanges` | `sessions.json`, `active-snapshot.json`, `active-trace-metadata.json`, `trusted-baseline.json` | Session、interrupted-work state、installer metadata、user-created baseline。Atomic write で、session history は最大 100 です。Loader は unreadable な legacy-format data を quarantine せず、空または `nil` を返します。 |
